<?php
/**
 * @version		$Id: script.deallab.php 2013-07-17 studio4 $
 * @package		DealLab Install script
...
class Com_DealLabInstallerScript
{
    public function postflight($type, $parent)
    {
        ...
        foreach ($modules as $module)
        {
			...
			{
				$db->setQuery("UPDATE #__modules SET `position`=".$db->quote('cpanel').",`published`='1' WHERE `module`=".$db->quote($name));
				$db->query();
    ...
    {
		echo '<p>DealLab update is successful.</p>';
    }
	
    private function installationResults($status)
    {
        $language = JFactory::getLanguage();
        $language->load('com_deallab');
        $rows = 0; ?>
        <img src="<?php echo JURI::root(true); ?>/media/deallab/assets/icons/system/deallab_48x48.png" alt="DealLab" align="right" />
        ...
        <table class="adminlist table table-striped">
            ...
            <tbody>
                ...
                <tr class="row<?php echo(++$rows % 2); ?>">
                    <td class="key"><?php echo $module['name']; ?></td>
                    <td class="key"><?php echo ucfirst($module['client']); ?></td>
                    <td><strong><?php echo ($module['result'])?'Removed':'Not removed'; ?></strong></td>
                </tr>
                <?php endforeach; ?>
                <?php endif; ?>
